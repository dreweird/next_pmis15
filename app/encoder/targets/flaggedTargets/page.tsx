...
const page = () => {
   ...
   const handleFlagged = async (mfo_id: any) => {
    const result = window.confirm("Are you sure you want to resubmit the target?");
    if(result){
      const response = await fetch("/api/mfo/update", {
        ...
        body: JSON.stringify({ mfo_id, col_name: "flagged", value: 0 }),
      });
      const json = await response.json();
      if(!json){
        alert("Something went wrong!")
      }else{
        alert("resubmission was successful!")
        if (gridRef.current) {
    ...
    useEffect(() => {
      fetch("/api/mfo/listFlagged") // Fetch data from server
        ...
        const [colDefs, setColDefs] = useState<(ColDef | ColGroupDef)[]>([
          ...
          {headerName: "Reviewer's Remarks", field: 'reviewer_remarks', minWidth: 200},
