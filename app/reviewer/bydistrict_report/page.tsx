...
import { AllCommunityModule, ClientSideRowModelModule, ColDef, ColGroupDef, ExcelExportModule, ModuleRegistry, PivotModule, RowGroupingModule, RowSelectionModule, themeBalham, TreeDataModule } from 'ag-grid-enterprise';
...
const district = () => {
  ...
  const hancleClick = () => {
    fetch(`/api/byDistrict/${selectedValue}/${selectedValueProv}/${selectedValueDist}`) // Fetch data from server
      ...
      const [colDefs, setColDefs] = useState<(ColDef | ColGroupDef)[]>([
       ...
       // { field: 'h3',  rowGroup: true, hide: true},
        ...
        {headerName: "Physical Target",  marryChildren: true,
          children: [
            {headerName: "Target", field: 'target', minWidth: 50, editable: false, valueFormatter: custom.currencyFormatter,  cellStyle: custom.customStyleGroupQuarter, aggFunc: 'sum', cellClass: ['data', 't']},
            {headerName: "Location", field: 'municipal', minWidth: 50, editable: false,  cellStyle: custom.customStyleGroupQuarter, cellClass: ['data']},
          ]
        },
        {headerName: "Physical Accomplishment",  marryChildren: true,
          children: [
  ...
  return (
    <div>
      <div className="flex items-stretch ...">
        ...
        <button onClick={hancleClick} className='bg-blue-700 shadow hover:bg-blue-500 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded'>Generate Report</button>
    </div>
