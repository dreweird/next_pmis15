...
const Navbar = () => {
  ...
  return (
    <nav className="flex items-center flex-wrap text-white bg-blue-700">
      <div className="flex w-full items-center justify-between my-2">        <Link className="font-bold px-4 bg-blue-700 text-white text-sm  py-2 rounded-md cursor-pointer hover:bg-blue-600" href="/">
          Home
        </Link>
        {session?.user && session?.user.email != 7 && session?.user.email != 0 && session?.user.email != 8 &&   <div className="flex items-center gap-x-5 ">
          <Link href="/encoder/targets" className="bg-blue-700 text-white text-sm px-4 py-2 rounded-md cursor-pointer hover:bg-blue-600">Targets 2026</Link>
