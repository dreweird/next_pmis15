...
const Navbar = async () => {
  ...
  return (
    <nav className="flex items-center flex-wrap text-white bg-blue-700">
      ...
      {session?.user && session?.user.email != 7 && session?.user.email != 0 && session?.user.email != 8 &&  <>
      ...
      <Link href="/encoder/accomplishment" className="bg-blue-700 text-white text-sm px-4 py-2 rounded-md cursor-pointer hover:bg-blue-600">Accomplishment</Link>
        ...
        {session?.user && session?.user.email === 0 &&  <Link href="/reviewer">BEDs123</Link>}
        {session?.user && session?.user.email === 7 &&  <Link href="/budget">Budget</Link>}
        {session?.user && session?.user.email === 8 &&  <Link href="/disburse">Disburse</Link>}
